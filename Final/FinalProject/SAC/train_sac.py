...
class Actor(nn.Module):
    ...
    def get_action_log_prob(self, state, stochstic=True):
        ...
        gaussian_log_prob = -0.5 * ((u - mu) ** 2 / (var + 1e-6) + 2 * torch.log(std + 1e-6))\
            .sum(dim=-1, keepdim=True) - 0.5 * mu.shape[-1] * math.log(2 * math.pi)
        log_prob = gaussian_log_prob - torch.log(self.k * (1 - (action / self.k) ** 2 + 1e-6))\
...
class SACAgent(nn.Module):
    def __init__(self, env, device, batch_size=256, print_period=20, save_period=1000000):
        ...
        self.total_step = 0
        self.max_episode_time = 10000

        self.save_model_path = 'SAC/model/'
        ...
        self.H_bar = torch.tensor([-self.action_dim]).to(self.device).float()
        self.alpha = ALPHA
    ...
    def soft_update(self, local_model, target_model, tau):
        ...
        for layer_name in target:
            target[layer_name] = (1 - tau) * target[layer_name] + tau * local[layer_name]
    ...
    def train(self, max_episode_num=1000, max_time=1000):
        ...
        for episode_idx in range(max_episode_num):
            ...
            for t in range(1, max_time + 1):
                ...
                if len(self.memory) < self.batch_size or t % NUM_TIME_STEP != 0:
                    continue
            ...
            if (episode_idx + 1) % self.print_period == 0:
                print('Tot_step: {} \t | Episode: {} \t | Reward : {} \t'
                      .format(self.total_step, episode_idx + 1, np.mean(self.rewardlist[-self.print_period:])))
